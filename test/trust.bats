...
SCRIPT="$BATS_TEST_DIRNAME/../scripts/trust.sh"

@test "creates trust file and adds path when file does not exist" {
  rm -f "$CLAUDE_PLUGIN_DATA/trusted-projects.json"

  run "$SCRIPT" "/tmp/my-project" "$CLAUDE_PLUGIN_DATA/trusted-projects.json"
  assert_success

  local paths
  paths=$(jq -r '.[]' "$CLAUDE_PLUGIN_DATA/trusted-projects.json")
...
@test "appends path to existing trust file" {
  ...
  run "$SCRIPT" "/tmp/new-project" "$CLAUDE_PLUGIN_DATA/trusted-projects.json"
  assert_success

  local count
  count=$(jq 'length' "$CLAUDE_PLUGIN_DATA/trusted-projects.json")
  assert_equal "$count" "2"

  local last
  last=$(jq -r '.[-1]' "$CLAUDE_PLUGIN_DATA/trusted-projects.json")
...
@test "creates parent directory if needed" {
  ...
  run "$SCRIPT" "/tmp/my-project" "$nested/trusted-projects.json"
  assert_success
  assert [ -f "$nested/trusted-projects.json" ]
}
